...
names(Consumo2)<-c('Año', 'C', 't', 'C.Con1')
View(Consumo2)

#gráfica de dispersión

x11()
plot(Consumo2$C.Con1~ Consumo2$t, main='gráfica de dispersión del consumo de energía\neléctrica en kWh/mes del departamentento del Vichada',lwd=5, xlab='tiempo en años contados a partir de 2004', ylab='Consumo', pch=19)


regresion1<-lm(Consumo2$C~Consumo2$C.Con1)
regresion1
...
x11()
plot(Consumo2$C ~ Consumo2$t, main='Regresión Crecimiento logistico del consumo de energía\neléctrica en kWh/mes del departamentento del Vichada',lwd=5, xlab='tiempo en años contados a partir de 1985', ylab='Consumo', pch=19)
lines(seq(0, 12, 1),1.002*C.t(C01, r1, k1, seq(2004, 2016, 1),A1)+45472.338, col='blue', lwd=3)
text(8,800000,substitute("C(t)"==paste(frac(paste("3151910"), paste(1+3.99*"e"^"-0.144t")))+45472.338),cex=1.5) 
...
a<-2019:2039
b<-C.t(P01, r1, k1, seq(2019, 2039, 1),A1)
